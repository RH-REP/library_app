...
- [x] 現在の PDF / image sample をレンダリングと OCR で確認する。
- [x] 文字が散らばる sample の文字化方針を整理する。
- [x] `metadata_header_contract.md` を読み、`organized_data/{item_id}/index.md` の header 契約を取り込む。
- [x] 実装工程を 3 段階レビュー方式に再構成する。
- [x] Review 1 用 report と手元確認コマンドを作成する。
- [x] Phase 0 に戻り、元文章から HTML / PDF / JPG を逆生成する確認 demo を作成する。
- [x] 逆生成 demo に、期待される `plain_text.txt`、`extraction_record.json`、
  `structured_text.json`、metadata header 付き organized example を追加する。
...
- PDF sample は text layer を持つが、2 段組と図が混ざるため、座標ベースで本文、caption、figure text を分ける。
- 画像 sample は通常 OCR の精度が低いため、diagram transcription を正式な検索用テキスト候補にする。
- metadata header は `plain_text.txt` ではなく `organized_data/{item_id}/index.md` の冒頭にだけ付ける。
- extractor と organizer を分け、`extracted_text/` と `organized_data/` の境界を維持する。
- 実装は Review 1: Contract Review、Review 2: Extractor Output Review、
  Review 3: Organized Data Review の 3 gate で進める。
- 実装の前に、`phase0_reverse_sample_demo/` を使って
  source text -> generated source -> expected extraction -> organized item の流れを確認する。
