...
## 2026-07-13 follow-up: 3-stage implementation process and metadata header

ユーザーの確認により、実装前に工程を 3 段階レビュー方式へ切ることにした。
...
1. Review 1: Contract Review
   contract、schema、metadata header adapter、保存境界を確認する。
2. Review 2: Extractor Output Review
   HTML / PDF / image の実サンプル抽出結果を確認する。
3. Review 3: Organized Data Review
   metadata header 付き `organized_data/{item_id}/index.md` と source refs を確認する。

実装 Phase は各 review の前後に配置する。
...
Issue #6 の追加コメントに合わせて、Phase 1 を機能単位でサブエージェントに分担した。
worker は統合と検証を担当した。
...
- `plain_text.txt`: 検索投入用の合成 text。
- `extraction_record.json`: source、extractor、status、method、warning などの machine metadata。
- `structured_text.json`: `body_text`, `caption_text`, `figure_text`,
  `diagram_transcription` を分けて保存する structured text。

重要な判断:

- `figure_text` は図内 OCR 候補であり、default では `plain_text.txt` に混ぜない。
