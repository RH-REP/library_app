...
ユーザーの追加コメントに合わせて、自作側の synthetic fixture は削除する。
今後の sample 入力は形式別の `source_samples/html/`、`source_samples/pdf/`、
`source_samples/image/` と
`records/actual_web_source_samples.json` に寄せる。

### 作業項目

- [x] synthetic HTML / PDF / image sample を削除する。
- [x] synthetic sample index を削除する。
- [x] README と artifact の現在状態を actual source 中心へ更新する。
- [x] ユーザー向け issue log を追加する。

### この時点での判断

- 自作 sample file は残さない。
- 実装検証では actual web source set を優先して使う。
- synthetic sample の作成経緯は履歴として issue log に残すが、現行 sample set としては扱わない。

## 2026-07-13 follow-up 5: Issue #5 画像 sample 差し替え

### 方針

画像 sample は、プログラミング知見に直接関係する図表へ差し替える。
OCR / preview 検証では、イベント写真ではなくソフトウェア開発プロセス図を使う。

### 作業項目

- [x] 古いソフトウェアテスト大会の JPEG を削除する。
- [x] Royce final model の PNG を保存する。
- [x] provenance index と関連文書の参照パスを更新する。

### この時点での判断

- 画像 sample は「写真」ではなく「図表」を優先する。
- OCR の first cut では、図中テキストを抽出対象として扱う。
